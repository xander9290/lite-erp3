...
function StockPickingListView() {
  ...
  return (
    <ListView model="stockPicking">
      ...
      <ListView.Body>
        <TableTemplateLite pageSize={100} onRowClick={(row) => router.push(`/app/stock_picking?view_type=form&id=${row.id}`)} defaultOrder="name desc" model="stockPicking">
          ...
          />
          <Column field="Partner.name" label="Contacto" include={{ Partner: { select: { id: true, name: true } } }} />
          <Column field="Warehouse.description" label="Origen" include={{ Warehouse: { select: { id: true, description: true } } }} />
          <Column field="WarehouseDest.description" label="Destino" include={{ WarehouseDest: { select: { id: true, description: true } } }} />
