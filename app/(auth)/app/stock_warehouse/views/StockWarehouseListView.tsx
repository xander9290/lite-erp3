...
function StockWarehouseListView({ productId, whId, companyId }: { productId: string | null; whId: string | null; companyId: string | null }) {
  ...
  if (companyId) domain.push(["Warehouse.companyId"]);
      ...
      <ListView.Body>
        <TableTemplateLite model="stockWarehouse" defaultOrder="qty desc" baseDomain={domain} pageSize={100} showTotals={true} totalColumns={["qty"]}>
          ...
          <Column
            ...
            include={{
              Product: {
                select: { name: true, Uom: { select: { name: true } } },
              },
            }}
          />
