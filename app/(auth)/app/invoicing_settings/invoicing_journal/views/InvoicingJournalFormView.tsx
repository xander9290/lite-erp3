...
import { invoicingJournalSchema, invoicingJournalSchemaDefault, InvoicingJournalSchemaType } from "../schema/invoicingjournal.schema";
import { zodResolver } from "@hookform/resolvers/zod";
import { useRouter } from "next/navigation";
import { useEffect, useRef } from "react";
import { useModals } from "@/contexts/ModalContext";
import { createInvoicingJournal, InvoicingJournalWithProps, updateInvoicingJournal } from "../actions/invoicingjournal.action";
import { FieldBoolean, FieldEntry, FieldRelation, FieldSelect } from "@/components/templates/fields";
import { toast } from "react-hot-toast";

function InvoicingJournalFormView({ id, journal }: { id: string | null; journal: InvoicingJournalWithProps | null }) {
  ...
  const onSubmit: SubmitHandler<InvoicingJournalSchemaType> = async (data) => {
    if (id && id === "null") {
      ...
      router.replace(`/app/invoicing_settings/invoicing_journal?view_type=form&id=${res.data?.id}`);
  ...
  return (
    <FormView id={id} methods={methods} onSubmit={onSubmit} reverse={handleReverse} cleanUrl="/app/invoicing_settings/invoicing_journal?view_type=form&id=null">
      <FormViewGroup>
      ...
      <FormViewGroup>
        ...
        <FormViewStack>
          <FieldRelation model="invoicingCurrency" name="currencyId" label="Moneda" domain={[["active", "=", true]]} />
