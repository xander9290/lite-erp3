...
import { PartnerChildrenType, partnerSchema, partnerSchemaDefault, PartnerSchemaType } from "../schemas/partner.schema";
import { useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";
import { useModals } from "@/contexts/ModalContext";
import { FormView, FormViewGroup, FormViewStack } from "@/components/templates/FormView";
import { FieldAction, FieldBoolean, FieldEntry, FieldImage, FieldRelation, FieldSelect, FieldTags } from "@/components/templates/fields";
...
import ModalCreateChild from "./ModalCreateChild";
...
function PartnersFormView({ display, partner, id }: { display: string; partner: PartnerWithProps | null; id: string | null }) {
  ...
  const { remove, append: appChild } = useFieldArray({
    control,
    name: "Children",
  });
  ...
  const goToParent = () => {
    return router.push(`/app/partners?view_type=form&display=${partner?.Parent?.displayType}&id=${partner?.Parent?.id}`);
  };

  const submitNewChild = (data: PartnerChildrenType) => {
    appChild(data);
  };
        ...
        actions={[
          {
            ...
            invisible: ["CUSTOMER", "INTERNAL", "SUPPLIER"].includes(getValues().displayType),
          },
        ]}
      >
        <FormViewGroup>
        ...
        <Notebook defaultActiveKey="children">
          <Page eventKey="children" title="Contactos y direcciones">
            <Container fluid>
              <Row>
                <Col md="12">
                  <FieldAction className="my-1" name="actionCreateChild" action={() => setModalChild(!modalChild)} label="Crear" />
                </Col>
              </Row>
              <Row>
          ...
          <Page eventKey="otherInfo" title="Otra información">
            <PageSheet name="otherInfo">
              <FormViewGroup>
                ...
                />
                <FieldRelation name="parentId" label="Relacionado" model="partner" domain={[["displayType", "in", ["CUSTOMER"]]]} />
              </FormViewGroup>
            </PageSheet>
          </Page>
        </Notebook>
      </FormView>
      <ModalCreateChild submitNewChild={submitNewChild} onHide={() => setModalChild(!modalChild)} show={modalChild} />
    </>
  );
}
