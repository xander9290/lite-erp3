          ...
          <PageSheet name="purchaseOrderLine">
            <Col md="12" className="p-0 m-0 overflow-auto">
              <SimpleTable
                ...
                resizable
                headers={[
                  { string: "Producto", name: "productId", width: 200, minWidth: 100 },
                  { string: "Cantidad", name: "quantity", width: 30, minWidth: 30 },
                ...
                renderRow={(row, index) => (
                  <tr key={row.id} className="border-bottom">
                    <SimpleTD colIdx={index} name="lineProductId">
                      <FieldRelation
                        ...
                        searchColumns={[
                          ...
                          { field: "purchases", label: "Compra", type: "boolean" },
                        ]}
                        ponChange={(value, record) => computeProductLine({ value, record: record as ProductTemplate, line: index })}
                      />
                    </SimpleTD>
                    <SimpleTD colIdx={index} name="lineQuantity">
                      <FieldEntry inline name={`OrderLines.${index}.quantity`} type="number" decimals={3} onChange={(value) => computeQuantityLine({ value: Number(value), line: index })} />
                    </SimpleTD>
                    ...
                    <SimpleTD colIdx={index} name="linePriceUnit">
                      <FieldEntry inline name={`OrderLines.${index}.priceUnit`} type="number" decimals={2} onChange={(value) => computePriceUnit({ value: Number(value), line: index })} />
                    </SimpleTD>
