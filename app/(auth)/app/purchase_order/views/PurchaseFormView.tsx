...
function PurchaseFormView({ id, purchase }: { id: string | null; purchase: PurchaseOrderWithProps | null }) {
  ...
  useEffect(() => {
    ...
    const values: PurchaseOrderSchemaType = {
      ...
      confirmedDate: purchase.confirmedDate ? toDateTimeLocal(purchase.confirmedDate) : null,
      doneDate: purchase.doneDate ? toDateTimeLocal(purchase.doneDate) : null,
      ...
      warehouseAffectedId: {
        id: purchase.WarehouseAffected?.id || "",
        name: purchase.WarehouseAffected?.name || "",
      },
      currencyId: {
        id: purchase.Currency.id,
      ...
      StockPicking: {
        id: purchase.StockPicking?.id,
        name: purchase.StockPicking?.name,
      },
    };
    reset(values);
  ...
  useEffect(() => {
    ...
    const setWarehouse = async () => {
      ...
      if (getCompany) {
        ...
        const getSaleWh = getCompany.Warehouses.filter((wh) => wh.type === "SALES")[0];
        if (getSaleWh) {
          setValue(`warehouseAffectedId`, {
            id: getSaleWh.id,
            name: getSaleWh.name,
          });
        }
  ...
  const actionConfirm = handleSubmit(async () => {
    ...
    const res = await confirmStockWarehousePurchase({
      data: { ...newData, id },
    });
    if (!res.success) return modalError(res.message);
  ...
  const actionCancel = handleSubmit(async () => {
    ...
    if (currentState === "purchase") {
      const res = await cancelStockWarehousePurchase({
        data: newData,
        orderId: id,
      });
      if (!res.success) return modalError(res.message);
    ...
    >
      <FormViewGroup>
        <FieldRelation
          ...
          ponChange={(_, record) => onChangeSupplier(record as PartnerWithProps)}
        />
        <FormViewStack>
          <FieldRelation
            model="warehouse"
        ...
        <FormViewStack>
          ...
          <FieldRelation model="invoicingCurrency" name="currencyId" label="Moneda" readonly={getValues().state !== "draft"} />
        </FormViewStack>
      </FormViewGroup>
      <FormViewGroup>
        <FormViewStack>
          <FieldEntry name="datePlanned" label="Fecha esperada" type="date" min={todayDate()} readonly={["done", "cancel"].includes(getValues().state)} />
          <FieldEntry name="date" label="Fecha" type="date" readonly />
      ...
      <Notebook defaultActiveKey="orderLine">
        ...
        <Page eventKey="otherInfo" title="Otra información">
          <PageSheet name="otherInfoPage">
            ...
            <FormViewGroup>
              <FormViewStack>
                <FieldEntry name="createdAt" label="Creado" type="datetime-local" readonly />
                <FieldEntry name="confirmedDate" label="Confirmado" type="datetime-local" readonly />
                <FieldEntry name="doneDate" label="Terminado" type="datetime-local" readonly />
              </FormViewStack>
            </FormViewGroup>
          </PageSheet>
        </Page>
