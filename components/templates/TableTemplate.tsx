...
//   // ── Stable ref for external domain to avoid JSON.stringify in deps ──
//   const externalDomainRef = useRef(externalDomain);
//   useEffect(() => {
//     // Only reset page when the domain actually changes by value
//     if (
...
//   const visibleColumns = columns; // filter(() => true) was a no-op
...
//   const handleHeaderDoubleClick = useCallback((key: string) => {
//     setSortConfig((prev) => ({
//       key,
...
//     setNewFilterValue("");
//     setPage(1);
//   }, [newFilterField, newFilterValue, newFilterOperator, columnMap]);
...
//   const groupedData = useMemo(() => {
//     if (!groupBy) return { null: rows } as Record<string, T[]>;
//     const col = columnMap.get(groupBy);
//     if (!col) return { null: rows } as Record<string, T[]>;
